...
find . -type f -exec sed -i "s/laravel-package-template/$package_kebab_case/g" {} +

# Replace Pascal cases
find . -type f -exec sed -i "s/AuthorTemplate/$author_pascal_case/g" {} +
find . -type f -exec sed -i "s/LaravelPackageTemplate/$package_pascal_case/g" {} +

# Rename files
find . -type f -name "*LaravelPackageTemplate*" -exec sh -c 'mv "$1" $(echo "$1" | sed "s/LaravelPackageTemplate/$2/")' _ {} "$package_pascal_case" \;
find . -type f -name "*laravel-package-template*" -exec sh -c 'mv "$1" $(echo "$1" | sed "s/laravel-package-template/$2/")' _ {} "$package_kebab_case" \;
